...
export function ScheduleDayContent({ startDateTime, description }: ScheduleDayContentProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full m-4">
      ...
      <p className="text-2xl text-tarot-100">{start.monthShort} {start.day}<sup>{ordinal(start.day)}</sup> at {start.hour % 12} {start.hour >= 12 ? 'PM' : 'AM'} {start.toFormat('ZZZZ')}</p>
